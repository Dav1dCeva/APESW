...
exports.createClienteCallback = createClienteCallback;
exports.updateClientePromise = updateClientePromise;
exports.getClienteById = getClienteById;
exports.getAllClientes = getAllClientes;
exports.deleteCliente = deleteCliente;
exports._clearClientesForTest = _clearClientesForTest;
...
function validateClientData(data) {
    if (!data)
        return 'Datos de cliente vacíos';
    if (typeof data.nombre !== 'string' || data.nombre.trim().length === 0)
        return 'El nombre es obligatorio';
    if (typeof data.apellido !== 'string' || data.apellido.trim().length === 0)
        return 'El apellido es obligatorio';
    if (typeof data.dni !== 'string' || !/^\d{6,12}$/.test(data.dni))
        return 'DNI inválido (solo dígitos, 6-12 caracteres)';
 ...
 */
function createClienteCallback(newClient, callback) {
 ...
 */
function updateClientePromise(id, patch) {
...
function clientsNonNull(idx) {
    const rec = clientes[idx];
    if (!rec)
        throw new Error('Registro interno inconsistente');
    return rec;
}
 ...
 */
async function getAllClientes(includeDeleted = false) {
    await new Promise(r => setTimeout(r, 80));
 ...
 */
async function deleteCliente(id, options) {
...
function _clearClientesForTest() {
    clientes.length = 0;
    nextId = 1;
}
